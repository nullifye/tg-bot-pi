return function(msg)
  ...
  if args[1]==cmd then
    if #args == 2 then
      curr_ptt = os.time()
      try = os.execute('wget -qO- "http://api.pos.com.my/v2TrackNTraceWebApi/api/Details/'..args[2]..'?sKey=7oHHMvxM0" --connect-timeout='..TIMEOUT..' | grep -Po \'"date":.*?[^\\\\]",|"process":.*?[^\\\\]",|"office":.*?[^\\\\]",|},{\' | awk -F\'"[:]"\' \'{print $2}\' | sed -e "s/\\",//" | sed -e \':a;N;$!ba;s/\\n\\n/\\n----------\\n/g\' > '..TMP_PATH..'/ppos'..curr_ptt..'.out')

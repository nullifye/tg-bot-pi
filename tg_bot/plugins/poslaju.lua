return function(msg)
  ...
  if args[1]==cmd then
    if #args == 2 then
      ...
      if try then
	    send_text (replyto(msg), TMP_PATH.."/ppos"..curr_ptt..".out", ok_cb, false)
      end

	  send_msg (replyto(msg), "--end of request", ok_cb, false)
    else
      send_msg (replyto(msg), "usage: pi:poslaju <TRACKING-NO>", ok_cb, false)
    end
    return true
  end
end
